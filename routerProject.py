...
class NetworkTopo( Topo ):
	...
	# Define networks and router
        ...
        dmz = '172.16.0.1/12'
        ext = '147.0.0.1/12'
        router = self.addNode( 'r0', cls=LinuxRouter, ip=subnet1 )
    ...
    # Add hosts
        s1u1 = self.addHost( 's1u1', ip='192.168.0.101/24',
                           defaultRoute='via 192.168.0.1' )
        s1u2 = self.addHost( 's1u2', ip='192.168.0.102/24',
                           defaultRoute='via 192.168.0.1' )
        s2u1 = self.addHost( 's2u1', ip='192.168.1.101/8',
                           defaultRoute='via 192.168.1.1' )
        s2u2 = self.addHost( 's2u2', ip='192.168.1.102/8',
                           defaultRoute='via 192.168.1.1' )
                          
    # Add DMZ hosts
        dmzserver1 = self.addHost( 'dmzserver1', ip='172.16.0.101/12',
                           defaultRoute='via 172.16.0.1' )
        dmzserver2 = self.addHost( 'dmzserver2', ip='172.16.0.102/12',
                           defaultRoute='via 172.16.0.1' )
                           
    # Add external users
        ext1 = self.addHost( 'ext1', ip='147.0.0.101/12',
                           defaultRoute='via 147.0.0.1' )
        ext2 = self.addHost( 'ext2', ip='147.0.0.102/12',
                           defaultRoute='via 147.0.0.1' )
        ext3 = self.addHost( 'ext3', ip='147.0.0.103/12',
                           defaultRoute='via 147.0.0.1' )
